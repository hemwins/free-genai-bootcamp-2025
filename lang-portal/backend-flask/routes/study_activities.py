...
def load(app):
    ...
    def get_study_activities():
        app.logger.info("Route hit: /api/study-activities GET")
        try:
            cursor = app.db.cursor()
            cursor.execute('SELECT id, name, url, preview_url FROM study_activities')
            
            activities = cursor.fetchall()
            app.logger.info(f"Retrieved {len(activities)} activities")
            ...
            app.logger.error(f"Error getting study activities: {str(e)}", exc_info=True)
